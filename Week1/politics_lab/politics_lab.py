...
def create_voting_dict():
    ...
    for a in voting_data:
        ...
        rv[splitted[0]]=[int(i) for i in splitted[3:]]
...
def most_similar(sen, voting_dict):
    ...
    """
    temp = {i:voting_dict[i] for i in voting_dict if i!=sen}
    rate=-1000;
    rv='nobody'
    for x in temp:
            level = policy_compare(sen,x,voting_dict)
            if level > rate:
               rv=x
               rate=level
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    count = 0
    for p in sen_set:
        average = average + sum([x*y for (x,y) in list(zip(voting_dict[sen],voting_dict[p]))])
        count = count+1
...
def just_dems():
    ...
    for a in rv:
        splitted = a.rstrip().split(' ')
        justDem[splitted[0]]=[int(i) for i in splitted[3:]]

#{ a for a in justDem if a!='Dayton'}
#find_average_similarity('Dayton',{ a for a in justDem if a!='Dayton'},justDem)
    return justDem
"""
for callout in {a for a in just_dems()}:
	tempAv = find_average_similarity(callout,{ g for g in just_dems() if g!=callout},just_dems())
	if tempAv > mostAv:
		mostAv=tempAv
		name = callout
		"""

most_average_Democrat = 'Reid' # give the last name (or code that computes the last name)
...
def find_average_record(sen_set, voting_dict):
    ...
    Example:
        ...
        [-0.5, -0.5, 0.0]
    """
    #add each component of each list
    #then divide that number by total number of things in list
    #retun the same size list for the result
    
        
    return [sum(x)/len(sen_set) for x in list(zip(*(voting_dict[x] for x in sen_set)))]
...
def bitter_rivals(voting_dict):
    ...
    name2=''
    split=10000
    for one in voting_dict:
        for two in voting_dict:
            if one!=two:
                #temp = sum(x*y for x in voting_dict[one] for y in voting_dict[two])
                policy = policy_compare(one,two,voting_dict)
                if policy < split:
                    split = policy
                    name1=one
                    name2=two
